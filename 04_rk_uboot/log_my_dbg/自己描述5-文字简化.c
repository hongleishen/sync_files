...
设备 device  ------------------// g_dnl_bind()            -> device_desc 初始化                   
        配置 config  ----------// g_dnl_config_register() -> *config 初始化;  usb_add_config() -> list_add_tail(&config->list, &cdev->configs);
                接口 interface-// g_dnl_do_config() -> fastboot_add() -> usb_function 初始化， usb_add_function() ->  list_add_tail(&function->list, &config->functions);   interface_desc
                                        // usb_function.descriptors = fb_hs_function()



设备 device  ------------------             
        配置 config  ----------//  list_add_tail(&config->list, &cdev->configs);            config   添加到  cdev->configs 列表
                接口 interface-//  list_add_tail(&function->list, &config->functions);      function 添加到  config-functions 列表    
                              //                                                            function->descriptors = fb_fs_function;  