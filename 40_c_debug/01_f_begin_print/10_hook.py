...
def  process_text(text):
    ...
    for index, line in enumerate(in_lines, start=1):
        ...
        # 处理注释
        if line.find("/*") != -1 or line.startswith(" *")  or line.startswith("*/") \
                or line.find("//") != -1:
            #print("find commit *")
            pre_line = tmp_line
            out_text += line + '\n'
        ...
        # 如果有 return 不能直接添加；  添加 f_end_hook(); 并且置位 f_has_hook
        # return 不换行;  a & !b & !c   ||   a & b & c
        # if line.find("return") != -1 and line.find(")") != -1:

        if (   line.find("return ") != -1 and line.find("(") == -1 and line.find(")") == -1) \
            or (line.find("return ") != -1 and line.find("(") != -1 and line.find(")") != -1):
            print("*********line = ", line)
            parts = line.split("return")
            print("find return at begin")
            line = parts[0] + '{f_end_hook();  ' + 'return' + parts[1] + '}'
            out_text += line + '\n'
            pre_line = tmp_line
            continue

        # return 换行  检查pre_line
        if pre_line.find("return ") != -1 and pre_line.find("(") != -1 \
                                         and pre_line.find(")") == -1 :
            ...
            parts = pre_line.split("return")
            line = parts[0] + '{f_end_hook();\n' + parts[0] +'return' + parts[1] + '\n' + line + '}'
            out_text += line + '\n'


            pre_line = "has re_turn"
            continue
    

        # 函数最后一行 "}" 不能直接添加 
        if not (ff1 and ff2 and ff3 and line.startswith("}")):  # 不是 } 行
            if line.find("return ") == -1:                       # 没有return
                print("add line", line)
                out_text += line + '\n'
        ...
        if ff1 and ff2 and ff3:
            if line.startswith("}"):
                ...
                print(f'----pre_line = {pre_line}')
                print(pre_line.find("re_turn"))
                print(pre_line.find("return "))
                # 没有re_turn 且   没有单行的return
                if (pre_line.find("re_turn") == -1 and pre_line.find("return ") == -1):
                    print("no return")
...
if __name__ == '__main__':
    skip_dirs = [r'00_c_code\simple\dir_b', r'dir_a']
